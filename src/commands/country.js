...
let correctCountryName, guesserId, theCountry = "";
let guessStreak = {
    highscore: 0,
    record: ""
}
...
function country(message) {
    ...
    if(command === `country`){
        ...
        theCountry = Math.floor(Math.random() * countries.length)
        let flag = countries[theCountry].flag
        ...
        correctCountryName = countries[theCountry].name
...
module.exports = {
    ...
    registerCommands: (commandHandler) => {
        commandHandler.set(`${config.prefix}country`, country)
    }
}